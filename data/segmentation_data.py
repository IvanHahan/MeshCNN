...
def show_edges(mesh, label=None,  colors=[[0,0,0,255], [120,120,120,255]]):
...
def show_mesh(mesh, label):
    ...
    for e_l, e in zip(label, edges):
        if e_l == 1:
            vertex_label[e] = 1
    faces = mesh.faces
    vertex_colors = np.array([[100, 255, 0, 255], [255, 0, 100, 255]])[vertex_label.astype(int)]
